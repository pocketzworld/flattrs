# automatically generated by the FlatBuffers compiler, do not modify

# namespace: flattrs_test

import flatbuffers
from flatbuffers.compat import import_numpy
np = import_numpy()

class AllScalarsWithDefaults(object):
    __slots__ = ['_tab']

    @classmethod
    def GetRootAs(cls, buf, offset=0):
        n = flatbuffers.encode.Get(flatbuffers.packer.uoffset, buf, offset)
        x = AllScalarsWithDefaults()
        x.Init(buf, n + offset)
        return x

    @classmethod
    def GetRootAsAllScalarsWithDefaults(cls, buf, offset=0):
        """This method is deprecated. Please switch to GetRootAs."""
        return cls.GetRootAs(buf, offset)
    # AllScalarsWithDefaults
    def Init(self, buf, pos):
        self._tab = flatbuffers.table.Table(buf, pos)

    # AllScalarsWithDefaults
    def Boolean(self):
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(4))
        if o != 0:
            return bool(self._tab.Get(flatbuffers.number_types.BoolFlags, o + self._tab.Pos))
        return True

    # AllScalarsWithDefaults
    def Uint8(self):
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(6))
        if o != 0:
            return self._tab.Get(flatbuffers.number_types.Uint8Flags, o + self._tab.Pos)
        return 1

    # AllScalarsWithDefaults
    def Uint16(self):
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(8))
        if o != 0:
            return self._tab.Get(flatbuffers.number_types.Uint16Flags, o + self._tab.Pos)
        return 1

    # AllScalarsWithDefaults
    def Uint32(self):
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(10))
        if o != 0:
            return self._tab.Get(flatbuffers.number_types.Uint32Flags, o + self._tab.Pos)
        return 1

    # AllScalarsWithDefaults
    def Uint64(self):
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(12))
        if o != 0:
            return self._tab.Get(flatbuffers.number_types.Uint64Flags, o + self._tab.Pos)
        return 1

    # AllScalarsWithDefaults
    def Int8(self):
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(14))
        if o != 0:
            return self._tab.Get(flatbuffers.number_types.Int8Flags, o + self._tab.Pos)
        return 1

    # AllScalarsWithDefaults
    def Int16(self):
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(16))
        if o != 0:
            return self._tab.Get(flatbuffers.number_types.Int16Flags, o + self._tab.Pos)
        return 1

    # AllScalarsWithDefaults
    def Int32(self):
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(18))
        if o != 0:
            return self._tab.Get(flatbuffers.number_types.Int32Flags, o + self._tab.Pos)
        return 1

    # AllScalarsWithDefaults
    def Int64(self):
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(20))
        if o != 0:
            return self._tab.Get(flatbuffers.number_types.Int64Flags, o + self._tab.Pos)
        return 1

    # AllScalarsWithDefaults
    def Float32(self):
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(22))
        if o != 0:
            return self._tab.Get(flatbuffers.number_types.Float32Flags, o + self._tab.Pos)
        return 1.0

    # AllScalarsWithDefaults
    def Float64(self):
        o = flatbuffers.number_types.UOffsetTFlags.py_type(self._tab.Offset(24))
        if o != 0:
            return self._tab.Get(flatbuffers.number_types.Float64Flags, o + self._tab.Pos)
        return 1.0

def AllScalarsWithDefaultsStart(builder): builder.StartObject(11)
def Start(builder):
    return AllScalarsWithDefaultsStart(builder)
def AllScalarsWithDefaultsAddBoolean(builder, boolean): builder.PrependBoolSlot(0, boolean, 1)
def AddBoolean(builder, boolean):
    return AllScalarsWithDefaultsAddBoolean(builder, boolean)
def AllScalarsWithDefaultsAddUint8(builder, uint8): builder.PrependUint8Slot(1, uint8, 1)
def AddUint8(builder, uint8):
    return AllScalarsWithDefaultsAddUint8(builder, uint8)
def AllScalarsWithDefaultsAddUint16(builder, uint16): builder.PrependUint16Slot(2, uint16, 1)
def AddUint16(builder, uint16):
    return AllScalarsWithDefaultsAddUint16(builder, uint16)
def AllScalarsWithDefaultsAddUint32(builder, uint32): builder.PrependUint32Slot(3, uint32, 1)
def AddUint32(builder, uint32):
    return AllScalarsWithDefaultsAddUint32(builder, uint32)
def AllScalarsWithDefaultsAddUint64(builder, uint64): builder.PrependUint64Slot(4, uint64, 1)
def AddUint64(builder, uint64):
    return AllScalarsWithDefaultsAddUint64(builder, uint64)
def AllScalarsWithDefaultsAddInt8(builder, int8): builder.PrependInt8Slot(5, int8, 1)
def AddInt8(builder, int8):
    return AllScalarsWithDefaultsAddInt8(builder, int8)
def AllScalarsWithDefaultsAddInt16(builder, int16): builder.PrependInt16Slot(6, int16, 1)
def AddInt16(builder, int16):
    return AllScalarsWithDefaultsAddInt16(builder, int16)
def AllScalarsWithDefaultsAddInt32(builder, int32): builder.PrependInt32Slot(7, int32, 1)
def AddInt32(builder, int32):
    return AllScalarsWithDefaultsAddInt32(builder, int32)
def AllScalarsWithDefaultsAddInt64(builder, int64): builder.PrependInt64Slot(8, int64, 1)
def AddInt64(builder, int64):
    return AllScalarsWithDefaultsAddInt64(builder, int64)
def AllScalarsWithDefaultsAddFloat32(builder, float32): builder.PrependFloat32Slot(9, float32, 1.0)
def AddFloat32(builder, float32):
    return AllScalarsWithDefaultsAddFloat32(builder, float32)
def AllScalarsWithDefaultsAddFloat64(builder, float64): builder.PrependFloat64Slot(10, float64, 1.0)
def AddFloat64(builder, float64):
    return AllScalarsWithDefaultsAddFloat64(builder, float64)
def AllScalarsWithDefaultsEnd(builder): return builder.EndObject()
def End(builder):
    return AllScalarsWithDefaultsEnd(builder)